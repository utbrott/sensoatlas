...
import { config, tasks, validation, Provider, useStore } from '@store/pressure'
...
import { PageHeader, Article } from '@data/laboratories/transducers/pressure'
import { Button } from '@ui/button'
import { getPressureSlope } from '@data/index'
...
const Charts = ({ tasksComplete }: ChartsProps) => {
  ...
  const { xvalue: xvalues, yvalues } = getPressureSlope({
    timeConstant: Number(dataStore.validation1)
  })
  ...
  return tasksComplete ? (
    <div className='flex h-full w-full flex-col space-y-4 rounded-md bg-gray-200/30 p-4 dark:bg-gray-800'>
      ...
      <span>
        <Tab.Group>
          <Tab.List>
            <Tab>{'Task 1: Iout= f(P)'}</Tab>
            <Tab>{'Task 2: Dynamic characteristic P = f(t)'}</Tab>
          </Tab.List>
          <Tab.Panels>
            <Tab.Panel>
              <LineChart
                chartName='pressure-out-current'
            ...
            <Tab.Panel>
              <LineChart
                chartName='pressure-dynamic-char'
